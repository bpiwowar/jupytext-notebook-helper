...
def test_config_is_read_from_the_tool_table(course):
    ...
    assert config.student_base_deps == ["cached-hub>=0.3.0"]
...
def test_a_constrained_requirement_wins_over_a_bare_one():
    assert merge_requirements(["cached-hub", "torch"], ["cached-hub>=0.3.0"]) == [
        "cached-hub>=0.3.0",
        "torch",
    ]
    # ... whichever side it comes from
    assert merge_requirements(["cached-hub>=0.3.0"], ["cached-hub"]) == [
        "cached-hub>=0.3.0"
    ]
...
def test_generated_project(course, tmp_path):
    ...
    assert '"cached-hub>=0.3.0",' in text
    assert '"cached-hub",' not in text
...
def test_command_line_adds_to_the_config(course, tmp_path):
    ...
    assert '"cached-hub>=0.3.0",' in text  # config still applies
